...
const ddTemplate = document.createElement('template');

ddTemplate.innerHTML = `
  <style>
    :host {
  ...
  <div class="dropdown">
    ...
    <anton-button as-atom>Content</anton-button>
...
class Dropdown extends HTMLElement {
  constructor() {
    ...
    this._sR.appendChild(ddTemplate.content.cloneNode(true));

    this.$label = this._sR.querySelector('.label');
    this.$button = this._sR.querySelector('anton-button');
  ...
  get option() {
    return this.getAttribute('option') || Object.keys(this.options)[0];
  }
...
(isChrome || isFirefox) && window.customElements && window.customElements.define('anton-dropdown', Dropdown);
